...
This article is a feasibility study for a class of automated market makers (AMMs) that passively provide liquidity at a price based on the uniform clearing price (UCP) of a competitive batch auction such as CoW protocol.
%
...
This article is not a whitepaper, specification, or proposal, and it makes no claim to exhaustiveness or completeness on any of the topics discussed --- particularly security. 
%
...
\end{itemize}
In the remainder of this report, we discuss approaches to implementing an AMM that fulfils these properties. It is structured as follows: in \S\ref{general}, we review general principles of designing CFMMs and pricing oracles; 
%
in \S\ref{architecture} we propose architectures, particularly as regards the structure of the channel through which the UCP is communicated to a Feedlot liquidity pool; 
%
...
\begin{itemize}
  \item
    \emph{Visibility.}
    Efforts should be made to precisely define manipulation in the context of CoW and Feedlot and to make activities falling under the definition \emph{detectable} if they occur, especially if carried out by or with the cooperation of solvers.
    
    General definitions of price manipulation have long proved elusive \cite{perdue1987manipulation,kyle2008define}.
    %
    Common themes are the importance of \emph{intent} --- difficult to establish --- and the concept of an `artificial' price, which is itself difficult to define.
    %
    We propose to 
    ...
    %
    It is not generally possible to prove that the owners of two addresses did or did not act coperatively, which we believe is a prerequisite for establishing intent.
    %
  ...
  \item
    ...
    We have proposed volume controls as a method for reducing the extent to which Feedlot can be used to subsidise manipulation.
    %
    While a na\"ive volume control in terms of the CoW volume cannot work by itself, a more sophisticated approach might.
    %
    We propose two methods for further exploration:
    \begin{itemize}
      \item In a similar manner to what we have done for price manipulation, develop a quantitative model to estimate the cost to manipulate $V_\mathrm{CoW}$ and devise methods to lower bound this.
      \item Supplement this approach with ad hoc absolute volume rules that can be set by the DAO.
    \end{itemize}
  ...
  \item
    ...
    The unique position of solvers also makes them particularly well-positioned to fight \emph{against} manipulation by atomically arbitraging manipulated prices into line with external markets in its solution.
    %
    For a solver whose payoffs do not depend on the output of the Feedlot oracle (in the sense that their equilbrium payoffs are not affected by the existence or non-existence of Feedlot), this strategy should even be pure profitable.
