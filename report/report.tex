...
%
We simulate portfolio data from February 2022 - March 2023. 
%
Results show that LP profitability is highly correlated between Feedlot and the CPMM. 
%
The Feedlot profitability could be thought of as leveraged CPMM profits due to the portfolio reserve composition skewing towards ETH.
[DATA GOES HERE]

%
There are limitations to our simulations. Without inventory balancing controls, the pool TVL is exposed to local order flow directionality as well as asset prices. 
%
Local order flow is probably hard to model and in real life would depend on how competitive the pool's prices are compared to other markets. 
%
Additionally, trade flow is aggregated between the single pair 5 and 30 bps Univ3 WETH/USDC pools and matched on CoW trade timestamps. This means the order flow is a coarser representation than the actual order flow occuring during the same time period. 
%
