...
If we restrict attention now to pairs that are dominated by settling on CFMMs (whose LPs are assumed not to collaborate with the adversary), we can rule out the ability --- even for the winning solver --- to manipulate prices for free.
%
Instead, the cost of manipulation becomes the cost of trading at the unfavourable manipulated price with non-allied LPs; that is, $V_\mathrm{AMM}\cdot(p - p_\mathrm{ext})$ where $V_\mathrm{AMM}$ is the volume of trades in the batch settling on AMMs, $p_\mathrm{ext}$ is the external market price, and $p$ is the manipulated price.
%
If the volume on Feedlot is allowed to exceed $V_\mathrm{AMM}$, price manipulation can still be profitable even in this setting.
